...
original contributor:
	John O'Brien III - http://www.linkedin.com/in/jobrieniii
contributors : 
...
function createArchive($keyword,$description,$tags,$screen_name,$user_id,$debug=false) {
	...
	// remove full text index EL
	//JN : add new attributes (from_user_name, from_user_lang) 
    $create_table = "CREATE TABLE IF NOT EXISTS `z_$lastid` (
        ...
        `from_user_name` varchar(30) character set utf8mb4 collate utf8mb4_unicode_ci NOT NULL,
        `from_user_location` varchar(40) character set utf8mb4 collate utf8mb4_unicode_ci NOT NULL,
        `from_user_url` varchar(512) NOT NULL,
        `from_user_description` varchar(320) character set utf8mb4 collate utf8mb4_unicode_ci NOT NULL,
        `from_user_created_at` varchar(50) NOT NULL,
        `from_user_verified` boolean NOT NULL,
        `from_user_contributors_enabled` boolean NOT NULL,
        `truncated` boolean NOT NULL,
        `in_reply_to_status_id` varchar(30) NOT NULL,
        `contributors` varchar(200) NOT NULL,
        `initial_tweet_id` varchar(100) NOT NULL,
        `initial_tweet_text` varchar(600) character set utf8mb4 collate utf8mb4_unicode_ci NOT NULL,
        `initial_tweet_user` varchar(60) character set utf8mb4 collate utf8mb4_unicode_ci NOT NULL,
        `initial_tweet_time` int(11) NOT NULL,
        `user_mentions` varchar(1000) NOT NULL,
        `urls` varchar(1200) NOT NULL,
        `medias_urls` varchar(1000) NOT NULL,
...
// create archive
function createArchive2($keyword,$type,$account_id,$description,$tags,$screen_name,$user_id,$debug=false) {
	global $db;
	$q = "select * from archives where keyword = '$keyword'";
	$r = mysql_query($q, $db->connection);
	if (mysql_num_rows($r) > 0) {
	...
	$q = "insert into archives values ('','$keyword','$type','$account_id','$description','$tags','$screen_name','$user_id','','".time()."','','',false)";
	$r = mysql_query($q, $db->connection);
	$lastid = mysql_insert_id();
	// remove full text index EL
	//JN : add new attributes (from_user_name, from_user_lang) 
...
function getTweets($id,$start=false,$end=false,$limit=false,$orderby=false,$nort=false,$from_user=false,$text=false,$lang=false,$max_id=false,$since_id=false,$offset=false,$lat=false,$long=false,$rad=false,$debug=false) {
	global $db;
   mysql_set_charset('utf8mb4',$db->connection);	
	$response = array();
...
function startProcess($cmd) {
	...
	/*
	if (stristr($cmd, 'crawl') != FALSE){
		$command = "$cmd > /home/www/tee2/log/crawl.log 2>&1 & echo $!";
		//$command = "$cmd > /dev/null 2>&1 & echo $!";
	}
	else {
		$command = "$cmd > /home/www/tee2/log/stream.log 2>&1 & echo $!";
	}
	*/
	
	$commandarray = explode("/", $cmd);
	$phpfile = $commandarray[count($commandarray)-1];
	$logarray = explode(".", $phpfile);
	$log = $logarray[0];
	$log = $log.".log";
	
	$command = "$cmd > ".$tk_your_dir."log/".$log." 2>&1 & echo $!";
